...
func ExportCsv(data []CloudflareIPData) {
    ...
    _ = w.Write([]string{"IP 地址", "已发送", "已接收", "丢包率", "平均延迟", "下载速度 (MB/s)", "地区码(Colo)"}) // 新增列
...
func (s DownloadSpeedSet) Print() {
   ...
   // 并在循环中调整格式
	...
	fmt.Printf(headFormat, "IP 地址", "已发送", "已接收", "丢包率", "平均延迟", "下载速度 (MB/s)", "地区码(Colo)")
	for i := 0; i < PrintNum; i++ {
		fmt.Printf(dataFormat, dateString[i][0], dateString[i][1], dateString[i][2], dateString[i][3], dateString[i][4], dateString[i][5], dateString[i][6]) // 7个参数
	}
	if !noOutput() {
